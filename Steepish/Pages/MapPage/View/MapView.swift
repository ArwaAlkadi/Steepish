...
/// Main gameplay screen: the walking-path map, HUD, popups, and puzzle presentation.
/// Supporting content, sheets, and puzzle logic live in the `MapView+*` extension files.
struct MapView: View {
    ...
    /// Identifies which bottom sheet is currently presented.
    ...
    /// Whether any full-screen cover is currently presented over the map.
    ...
    /// Scrolls the map to center on the caller's current position, once per appearance.
    ...
    /// Recursively searches a view hierarchy for the first sufficiently large `UIScrollView`.
